...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        # @comment: change with limulator
        self.client = limulator.Client()
        ...
        # @comment: Idk wtf is this for. Understand!
        # @comment: No agent will be loaded in my case?
    ...
    def _load_and_wait_for_world(self, town, ego_vehicles=None):
        ...
        # @comment: always reload world
        print(f'Town: {town}')
        self.world = self.client.load_world(town)

        # @comment: assuming async mode by default
        # if self._args.sync:
        #     settings = self.world.get_settings()
    ...
    def _load_and_run_scenario(self, config):
        ...
        """
        result = False
        # @comment: load the world and wait for it
        if not self._load_and_wait_for_world(config.town, config.ego_vehicles):
            self._cleanup()
            return False

        # @comment: skipping agent for now
        # if self._args.agent:
        ...
        # comment: TM is unnecessary for our purpose of simulating OSC 2.0
